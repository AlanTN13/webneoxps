...
function jsonLd(article) {
  ...
  const data = {
    ...
    "@type": article.contentType === "actualidad" ? "NewsArticle" : "BlogPosting",
...
function articleFallback(article) {
  return `<main data-static-seo="article"><article><nav><a href="/">NexOps</a> / <a href="/noticias">Insights</a></nav><p>${escapeHtml(article.category || article.contentType)}</p><time datetime="${escapeHtml(article.publishedAt)}">${escapeHtml(article.publishedAt)}</time><h1>${escapeHtml(article.title)}</h1><p>${escapeHtml(article.excerpt)}</p>${renderContent(article.content)}<p><a href="/noticias">Ver más Insights de NexOps</a></p></article></main>`;
}

function indexFallback(articles) {
  return `<main data-static-seo="index"><h1>NexOps Insights</h1><p>Actualidad, guías y análisis sobre automatización, IA, CRM y datos con foco en impacto empresarial.</p><section>${articles.map((article) => `<article><h2><a href="/noticias/${escapeHtml(article.slug)}">${escapeHtml(article.title)}</a></h2><p>${escapeHtml(article.excerpt)}</p></article>`).join("")}</section></main>`;
}
